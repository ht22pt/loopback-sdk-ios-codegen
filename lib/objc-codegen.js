...
exports.objcModels = function generateServices(app, modelPrefix, verbose) {
  ...
  for (var modelName in models) {
    ...
    if (models.hasOwnProperty(modelName)) {
      ...
      for (var relation in models[modelName].relations) {

        if (models[modelName].relations.hasOwnProperty(relation)) {
          var relationModel = models[modelName].relations[relation].model;
  ...
  var objcAllMTemplate   = readTemplate('./objc-all-h.ejs');
  ...
  var allScript = renderContent(objcAllMTemplate, models);
